...
class SignupForm extends Model
{
    ...
    public function SendWelcomeEmail($user,$profile)
    {

		
		$adminEmail = Yii::$app->params['adminEmail'];
		$adminmsg = "<table>";
		$adminmsg .= "<tr><td>Name</td><td>".$profile->fname." ".$profile->lname."</td></tr>";
		$adminmsg .= "<tr><td>Email</td><td>".$user->email."</td></tr>";
		$adminmsg .= "<tr><td>Phone</td><td>".$profile->phone."</td></tr>";
		$adminmsg .= "</table>";
		
		$msg = '<p style="margin:0;  text-align:left; padding:0;margin-bottom:5px;">';
		$msg .= "Greetings from Drish!<br><br>";
		$msg .= "This is to confirm that your account for www.Drish.com is now active.<br><br>";
		$msg .= "The next time you shop at Drish, you can save time at checkout by logging into your account. This will prefill your address information at the checkout.<br><br><br>";
		$msg .= "Thank you";
		$msg .= "</p>";		
		
		$sendtoadmin=  Yii::$app->mailer->compose(['html' => '@common/mail/views/contact'], ['name'=>'','subject'=>'A new customer has been registered','message'=>$adminmsg])
			->setTo($adminEmail)
			->setFrom('info@drish.com')
			->setSubject('New Registeration')
			->send();
			
			
		$sendtocustomer=  Yii::$app->mailer->compose(['html' => '@common/mail/views/contact'], ['name'=>$profile->fname.' '.$profile->lname,'subject'=>'Welcome to Drish!','message'=>$msg])
			->setTo($user->email)
